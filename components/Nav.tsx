...
export function Nav() {
  ...
  return (
    ...
    >
      <Link
        href="/"
        className="group flex items-center gap-2.5 sm:gap-3 select-none min-w-0 shrink-0"
      >
        <svg
          viewBox="0 0 32 32"
          aria-hidden
      ...
      <nav className="shrink-0 overflow-x-auto no-scrollbar">
        <div className="flex items-center p-1 rounded-full border border-ink-500/60 bg-ink-800/60 backdrop-blur-md text-[10px] sm:text-[11px] uppercase tracking-[0.16em] sm:tracking-[0.18em] min-w-max">
          {items.map((item) => {
            const active = pathname === item.href;
            return (
              <Link
                key={item.href}
                href={item.href}
                className="relative px-2.5 sm:px-3.5 md:px-4 py-1.5 text-bone-dim hover:text-bone transition-colors whitespace-nowrap"
              >
                <span className="relative z-10">{item.label}</span>
                {active && (
                  <motion.span
