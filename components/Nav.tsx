...
export function Nav() {
  ...
  return (
    <header className="px-4 md:px-10 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
      {/* Logo */}
      <Link
        href="/"
      ...
      >
        <svg
          viewBox="0 0 32 32"
          aria-hidden
          className="w-8 h-8 md:w-9 md:h-9 shrink-0 transition-transform duration-500 group-hover:rotate-[8deg]"
        >
          <rect width="32" height="32" rx="7" fill="rgba(255,255,255,0.03)" />

          {/* captured top-left box */}
          <rect
            x="7"
            y="7"
            width="9"
        ...
        <span className="italic-display text-lg md:text-xl leading-none tracking-tight">
          Pipopipette
        </span>
      </Link>
      ...
      <nav className="w-full sm:w-auto overflow-x-auto no-scrollbar">
        <div className="flex items-center justify-center sm:justify-start p-1 rounded-full border border-ink-500/60 bg-ink-800/60 backdrop-blur-md text-[10px] sm:text-[11px] uppercase tracking-[0.18em] min-w-max">
          {items.map((item) => {
            const active = pathname === item.href;
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="relative px-3 sm:px-4 py-1.5 text-bone-dim hover:text-bone transition-colors whitespace-nowrap"
              >
                <span className="relative z-10">{item.label}</span>
